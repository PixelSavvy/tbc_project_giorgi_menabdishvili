...
export const renderCourseCards = (courseData: CourseDataType[]) => {
  ...
  courseData.forEach((course) => {
    ...
    courseItem.innerHTML = generateCrouseCardHtml(course);
    courseList?.appendChild(courseItem);
  });
};

// Generate the HTML for course card
const generateCrouseCardHtml = (course: CourseDataType): string => {
  return `
